...
void build(int n, int st, int en)
{
    ...
    build(lc, st, mid);
    build(rc, mid + 1, en);
    tmn[n] = min(tmn[lc], tmn[rc]);
    tmx[n] = max(tmx[lc], tmx[rc]);
}
int rmx(int n, int st, int en, int l, int r)
{
    if (st > en || en < l || st > r)
        return INT_MIN;
    if (l <= st && en <= r)
        return tmx[n];
    int mid = (st + en) / 2, lc = 2 * n, rc = lc + 1;
    return max(rmx(lc, st, mid, l, r), rmx(rc, mid + 1, en, l, r));
}
int rmn(int n, int st, int en, int l, int r)
{
    if (st > en || en < l || st > r)
        return INT_MAX;
    if (l <= st && en <= r)
        return tmn[n];
    int mid = (st + en) / 2, lc = 2 * n, rc = lc + 1;
    return min(rmn(lc, st, mid, l, r), rmn(rc, mid + 1, en, l, r));
}
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0;; i++)
        {
            ...
            if (r == n)
                break;
            ans = max(ans, rmx(1, 0, n - 1, l, r) - rmn(1, 0, n - 1, l, r));
        }
        CASE(cs++);
        pfn(ans);
    }
