...
int f(int l, int r, int mode)
{
    ...
    if (!mode)
    {
        int cum = 0, ans = INT_MIN;
        Fre(i, l, r + 1)
        {
            cum += v[i];
            ans = max(cum + f(i + 1, r, 1), ans);
        }
        cum = 0;
        for (int i = r; i >= l; i--)
        {
            cum += v[i];
            ans = max(cum + f(l, i - 1, 1), ans);
        }
        return dp[l][r][mode] = ans;
    }
    else
    {
        int cum = 0, ans = INT_MAX;
        Fre(i, l, r + 1)
        {
            cum += v[i];
            ans = min(f(i + 1, r, 0), ans);
        }
        cum = 0;
        for (int i = r; i >= l; i--)
        {
            cum += v[i];
            ans = min(f(l, i - 1, 0), ans);
        }
        return dp[l][r][mode] = ans;
    }
}
