...
int f(int pos, int cnt, int cv)
{
    ...
    if (pos == len)
        return 1;
    if (dp[pos][cnt][cv] != -1)
        return dp[pos][cnt][cv];
    if (s[pos] == '?')
        return dp[pos][cnt][cv] = max(f(pos + 1, 1, 1 - cv), f(pos + 1, cnt + 1, cv));
    int ans;
    if (isvowel(s[pos]) == cv)
        ans = f(pos + 1, cnt + 1, cv);
    else
        ans = f(pos + 1, 1, 1 - cv);
    return dp[pos][cnt][cv] = ans;
}
int f2(int pos, int cnt, int cv)
{
    if (cv && cnt >= 3)
        return 1;
    if (!cv && cnt >= 5)
    ...
    if (dp[pos][cnt][cv] != -1)
        return dp[pos][cnt][cv];
    if (s[pos] == '?')
        return dp[pos][cnt][cv] = max(f2(pos + 1, 1, 1 - cv), f2(pos + 1, cnt + 1, cv));
    int ans;
    if (isvowel(s[pos]) == cv)
        ans = f2(pos + 1, cnt + 1, cv);
    else
        ans = f2(pos + 1, 1, 1 - cv);
    return dp[pos][cnt][cv] = ans;
}
