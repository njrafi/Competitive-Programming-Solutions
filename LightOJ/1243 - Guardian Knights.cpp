...
{
/// Number of edges * 2
/// Very careful with the number, be generous :p
const int MAX = 5010 * 2;
...
{
    if (x >= 0 && x < v.sz && y >= 0 && y < v.sz && v[x][y] != '#')
        return 1;
    return 0;
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        {
            //                                        cout << v[i][j] - 'A' + 1 << " " << i << " " << j << endl;
            knights[v[i][j] - 'A' + 1] = mp(i, j);
            //                  v[i][j] = '.';
        }
        int sou = 0;
        int sink = k + m + 1;
        mcmf::init(k + m + 2, sou, sink);
        Fre(i, 1, k + 1)
        {
            bfs(knights[i].ff, knights[i].ss);
            For(j, mills.sz)
            {
                int x = mills[j].ff;
                int y = mills[j].ss;
                if (dis[x][y] != -1)
                {
                    //                  cout << i << " " << j << " " << dis[x][y] << endl;
                    mcmf::addEdge(i, j + k + 1, 1, dis[x][y]);
                }
            }
        }
        Fre(i, 1, k + 1)
        {
            sf(inp);
            mcmf::addEdge(sou, i, inp, 0);
        }
        For(i, mills.sz)
            mcmf::addEdge(i + k + 1, sink, 1, 0);
        int ans = mcmf::solve().ff;
        CASE(cs++);
        pfn(ans);
