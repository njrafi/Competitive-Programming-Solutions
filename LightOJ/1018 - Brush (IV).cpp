...
pii getslope(int x, int y)
{
    // cout << x << " " << y << endl;
    pii ret;
    ret.ff = v[x].ff - v[y].ff;
    ret.ss = v[x].ss - v[y].ss;
    int gg = gcd(ret.ff, ret.ss);
    ret.ff /= gg;
    ret.ss /= gg;
    return ret;
}
int f(int mask)
{
    ...
    For(i, v.sz) if (!checkbit(mask, i))
    {
        ...
        ans = min(f(nmask) + 1, ans);
        For(j, v.sz) if (!checkbit(nmask, j))
        {
            int nnmask = mask | pre[i][j];
            ans = min(f(nnmask) + 1, ans);
        }
        break;
    }
    return dp[mask] = ans;
...
int main()
{
    ...
    while (t--)
    {
            ...
            For(j, v.sz) if (i != j)
        {
            ...
            int mask = 0;
            mask = setbit(mask, i);
            For(k, v.sz) if (k != i && getslope(i, k) == gg)
                mask = setbit(mask, k);
            pre[i][j] = mask;
        }
        CASE(++cs);
