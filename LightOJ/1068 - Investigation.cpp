...
int f(int pos, bool small, int mod1, int mod2)
{
    if (pos == v.sz)
    {
        mod2 %= k;
    ...
    if (vis[pos][small][mod1][mod2] == gg)
        return dp[pos][small][mod1][mod2];
    vis[pos][small][mod1][mod2] = gg;
    int hi = 10;
    if (!small)
        hi = v[pos] + 1;
    int ans = 0;
    For(i, hi)
    {
        int nmod1 = (mod1 * 10 + i) % k;
        int nmod2 = (mod2 + i) % k;
        if (i == hi - 1)
            ans += f(pos + 1, small, nmod1, nmod2);
        else
            ans += f(pos + 1, 1, nmod1, nmod2);
    }
    return dp[pos][small][mod1][mod2] = ans;
}
int main()
{
