...
long double deg(long double num)
{
    return num * 180 / pi;
}
int main()
{
    /*  cout << deg(atan2(-5,1)) << endl;
        cout << deg(atan2(-5,-1)) << endl;
        cout << deg(atan2(5,0)) << endl; */
    ...
    sort(all(v));
    For(i, v.sz - 1) if (v[i + 1].ff - v[i].ff < mn)
    {
        mn = v[i + 1].ff - v[i].ff;
        ans = i;
    }
    if (v[0].ff + 360 - v[v.sz - 1].ff < mn)
        cout << v[0].ss << " " << v[v.sz - 1].ss << endl;
    else
        cout << v[ans].ss << " " << v[ans + 1].ss << endl;
    return 0;
}
