...
#define inf 123456789101112
i64 dp[105][105][105];
int k, n, m;
vi v;
int col[105][105];
i64 f(int pos, int last, int part)
{
    // cout << pos << " " << last << " " << part << endl;
    if (part > k)
        return inf;
    if (pos == n)
    {
    ...
    if (dp[pos][last][part] != -1)
        return dp[pos][last][part];
    int nw = part;
    if (v[pos])
    {
        if (v[pos] != last)
            nw++;
        return dp[pos][last][part] = f(pos + 1, v[pos], nw);
    }
    else
    {
        i64 ans = inf;
        Fre(i, 1, m + 1)
        {
            ...
            if (i != last)
                nw++;
            ans = min(ans, f(pos + 1, i, nw) + col[pos][i]);
        }
        return dp[pos][last][part] = ans;
    }
}
int main()
{
