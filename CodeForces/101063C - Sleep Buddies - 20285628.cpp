...
int popcount(int mask, int m)
{
    int ans = 0;
    Fre(i, 1, m + 1) if (checkbit(mask, i))
        ans++;
    return ans;
}
int main()
{
    ...
    i64 n, m, q, mask, inp, type[3005] = {0};
    double k;
    cin >> n >> m;
    while (n--)
    {
    ...
    Fre(i, 1, 3000)
    {
        if (!type[i])
            continue;
        ans += type[i] * (type[i] - 1) / 2;
        Fre(j, i + 1, 3000)
        {
            if (!type[j])
                continue;
            int inter = popcount(i & j, m);
            int uni = popcount(i | j, m);
            double tem = (double)inter / (double)uni;
            if (tem >= k)
                ans += type[i] * type[j];
        }
    }
