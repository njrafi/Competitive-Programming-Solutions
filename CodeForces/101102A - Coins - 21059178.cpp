...
vi v1, v2;
i64 dp1[155][15005], dp2[155][15005];
i64 f1(int pos, int sum)
{
    if (sum == 0)
        return 1;
    if (pos == v1.sz)
    ...
    if (dp1[pos][sum] != -1)
        return dp1[pos][sum];
    return dp1[pos][sum] = (f1(pos + 1, sum) + f1(pos + 1, sum - v1[pos])) % mod;
}
i64 f2(int pos, int sum)
{
    if (sum == 0)
        return 1;
    if (pos == v2.sz)
    ...
    if (dp2[pos][sum] != -1)
        return dp2[pos][sum];
    return dp2[pos][sum] = (f2(pos + 1, sum) + f2(pos + 1, sum - v2[pos])) % mod;
}
int main()
{
    ...
    while (t--)
    {
        ...
        For(i, w + 1)
        {
            ...
            if (abs(s1 - s2) <= k)
            {
                ans = (ans + ((f1(0, s1) * f2(0, s2)) % mod)) % mod;
            }
        }
        cout << ans << endl;
    }
