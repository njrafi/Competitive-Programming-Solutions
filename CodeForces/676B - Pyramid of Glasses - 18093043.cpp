...
vector<double> tree[1000000];
int lstpar;
void update(int index, double quan)
{
    if (index > lstpar)
    {
        if (abs(tree[index][2] - 1) < eps)
            return;
        else
            tree[index][2] += quan;
        return;
    ...
    if (abs(tree[index][2] - 1) < eps)
    {
        update(tree[index][0], quan / 2);
        update(tree[index][1], quan / 2);
    }
    else
    {
        tree[index][2] += quan;
...
int main()
{
    ...
    while (t--)
        update(1, 1);
    int ans = 0;
    Fre(i, 1, n * (n + 1) / 2 + 1) if (abs(tree[i][2] - 1) < eps)
        ans++;
    cout << ans;
    return 0;
}
