...
int query2(int n, int st, int en, i64 rem)
{
    if (st == en)
        return st;
    int mid = (st + en) / 2, lc = 2 * n, rc = lc + 1;
    if (tree[lc].ff > rem)
        return query2(lc, st, mid, rem);
    else
        return query2(rc, mid + 1, en, rem - tree[lc].ff);
}
int main()
{
    int n, inp;
    ...
    //
    //        cout << query(1,1,n,1,n).ff << endl;
    i64 ans = 0;
    while (1)
    {
        while (query(1, 1, n, 1, n).ff > rem)
        {
            int firstFail = query2(1, 1, n, rem);
            //                        while( lo <= hi)
            //                        {
            //                                int mid = (lo+hi)/2;
            //
            //                                if( query(1,1,n,1,mid).ff > rem)
            //                                {
            //                                        firstFail = mid;
            //                                        hi = mid - 1;
            //                                }
            ...
            //                                        lo = mid + 1;
            //                        }
            //                        cout << "first Fail " << firstFail << endl;
            update(1, 1, n, firstFail, {0, 0});
            //                        cout << query(1,1,n,1,n).ff << endl;
            //                        getchar();
        }
        i64 totPrize = query(1, 1, n, 1, n).ff;
        i64 totCandy = query(1, 1, n, 1, n).ss;
        if (!totCandy)
            break;
        i64 totGhurbo = rem / totPrize;
        ans += totGhurbo * totCandy;
        rem -= totGhurbo * totPrize;
        //                cout << totPrize << " " << totCandy << " " << totGhurbo << endl;
    }
    cout << ans << endl;
    return 0;
}
