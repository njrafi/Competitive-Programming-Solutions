...
int dp[55][2][2][2], n, m, lim;
vs v;
int f(int h, int a, int b, int c)
{
    if (a && b && c)
        return 0;
    if (h == v.sz)
    ...
    if (dp[h][a][b][c] != -1)
        return dp[h][a][b][c];
    int ans = INT_MAX;
    For(i, m) if (isalpha(v[h][i]))
        ans = min(ans, f(h + 1, 1, b, c) + min(i, m - i));
    else if (isdigit(v[h][i]))
        ans = min(ans, f(h + 1, a, 1, c) + min(i, m - i));
    else ans = min(ans, f(h + 1, a, b, 1) + min(i, m - i));
    return dp[h][a][b][c] = ans;
}
int main()
{
