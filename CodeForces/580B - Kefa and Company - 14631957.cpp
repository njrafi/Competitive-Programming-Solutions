...
int main()
{
    ...
    For(i, money.sz)
    {
        auto u = upper_bound(all(money), money[i] + d - 1);
        auto l = lower_bound(all(money), money[i]);
        i64 up = u - money.begin();
        i64 down = l - money.begin();
        // cout << money[i] << " " << up << " " << down << endl;
        ans = max(ans, fr[up] - fr[down]);
    }
    cout << ans;
    return 0;
}
