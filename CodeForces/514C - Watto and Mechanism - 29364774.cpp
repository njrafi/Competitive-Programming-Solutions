...
int main()
{
    ...
    while (n--)
    {
        cin >> s;
        hash_calc();
        m[hash_lr(0, s.sz - 1)] = 1;
        //                cout << hash_lr(0,s.sz-1).ff << " " << hash_lr(0,s.sz-1).ss << endl;
        //                cout << hash_lr(0,1).ff << " " << hash_lr(3,4).ff << endl;
        //                cout << hash_lr(0,1).ss << " " << hash_lr(3,4).ss << endl;
    }
    while (q--)
    {
        cin >> s;
        ...
        For(i, s.sz)
        {
            For(j, 3)
            {
                ...
                pair<i64, i64> RR = hash_lr(i + 1, s.sz - 1);
                i64 hash1 = ((LL.ff * pr1[s.sz - i]) % mod1 + (ch * pr1[s.sz - 1 - i]) % mod1 + RR.ff) % mod1;
                i64 hash2 = ((LL.ss * pr2[s.sz - i]) % mod2 + (ch * pr2[s.sz - 1 - i]) % mod2 + RR.ss) % mod2;
                //                                if(i==2)
                //                                {
