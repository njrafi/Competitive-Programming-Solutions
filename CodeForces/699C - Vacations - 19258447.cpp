...
int f(int i, int last)
{
    ...
    if (dp[i][last] != -1)
        return dp[i][last];
    if (v[i] == 0 || v[i] == last)
        return dp[i][last] = f(i + 1, 0);
    if (last == 1 || last == 2)
        return dp[i][last] = max(1 + f(i + 1, 3 - last), f(i + 1, 0));
    else
    {
        if (v[i] == 1)
            return dp[i][last] = max(1 + f(i + 1, 1), f(i + 1, 0));
        if (v[i] == 2)
            return dp[i][last] = max(1 + f(i + 1, 2), f(i + 1, 0));
        else
            return dp[i][last] = max3(1 + f(i + 1, 2), 1 + f(i + 1, 1), f(i + 1, 0));
    }
}
int main()
{
