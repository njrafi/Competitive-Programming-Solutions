...
vpi v;
vi vb,w;
int tot,dp[1002][2020],cs,vis[1002][2020] , hi ;



int ff(int pos,int diff)
{
        if(   abs(diff)  > hi )
                return 12345678;
        ...
        if(vis[pos][diff+gg]==cs)
                return dp[pos][diff+gg];

        vis[pos][diff+gg] = cs;
        return dp[pos][diff+gg] = min(ff(pos+1,diff + v[pos].ss),ff(pos+1,diff-v[pos].ss));
...
bool f(int num)
{
        int ind = upper_bound(all(vb),num)  - vb.begin();
        hi = num;
        if(ind==vb.sz)
                return 1;
