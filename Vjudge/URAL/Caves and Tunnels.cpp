...
i64 query(int n,int st,int en,int l,int r)
{
        ...
        i64 q1 = query(lc,st,mid,l,r);
        i64 q2 = query(rc,mid+1,en,l,r);
...
int num[mxn];   /// not sure mybe the changed node number
...
i64 go(int a,int b)
{
        ...
        while( chain[a] != chain[b])
        {
                ...
                if(num[a] == num[start] + csz[chain[a]] - 1)
                        ans = max(ans, mx[chain[a]]);
                else
                        ans = max(ans, query(1,1,n,num[start],num[a]));
        ...
        ans = max(ans , query(1,1,n,num[a],num[b]));
...
void modify(int node,int val)
{
        ...
        mx[chain[node]] = query(1,1,n,st,en);
}
...
//        Fre(i,1,n+1)
//                cout << i << " " << chain[i] << " " << top[chain[i]] << endl;
        string s;
        sf(q);
        while(q--)
        {
