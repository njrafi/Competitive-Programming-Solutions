...
// The idea is to make sum S using N coins we can write f(S,N) = f(S,N-1) + f(S-v[N-1],N)

// it means we divide the problem into two sub problem : one without  the nth coin + one with the nth coin used once

// base cases are when sum will be zero it means this is one way , when sum will be minus 1 this is not a way

vi v;
i64 memo[105][1005];

i64 solve(i64 coins,i64 sum)
{
        if(sum==0)
                return 1;
        if (sum<0)
        ...
        return memo[coins][sum] = solve(coins-1,sum) + solve(coins,sum-v[coins-1]);
}
